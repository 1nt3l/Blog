...
<div id="results">
<?php
$month = protect($_GET['month']);
$year = protect($_GET['year']);
$archive_sql = mysql_query("SELECT * FROM blogger_archives WHERE month='$month' and year='$year'");
if(mysql_num_rows($archive_sql)==0) { header("Location: $url"); }
$archive = mysql_fetch_array($archive_sql);
$page = (int) (!isset($_GET["page"]) ? 1 : $_GET["page"]);
$limit = 15;
$startpoint = ($page * $limit) - $limit;
$sql = mysql_query("SELECT * FROM blogger_archives_posts WHERE archive_id='$archive[id]' ORDER BY id DESC LIMIT {$startpoint} , {$limit}") or die(mysql_error());
...
echo '<br>'; 
$num_rows = mysql_num_rows(mysql_query("SELECT * FROM blogger_archives_posts WHERE archive_id='$archive[id]'"));
$itemPerPage = $limit;
...
if($currentPage > $firstPage) {
	$pg = $currentPage-1;
	$load_page = "'$url','#results','archive','$pg','$archive[id]'";
